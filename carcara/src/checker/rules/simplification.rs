...
fn identity_of_op(pool: &mut dyn TermPool, op: &Operator, term: &Rc<Term>) -> Option<Term> {
    match op {
        ...
        Operator::FfAdd => match term.as_ref() {
            Term::Op(_, args) => {
                let Sort::Ff(order) = pool.sort(&args[0]).as_sort().cloned().unwrap() else {
                    unreachable!();
                };
                Some(Term::new_ffval(Integer::from(0), order))
            }
            _ => unreachable!(),
        },
        Operator::FfMul => match term.as_ref() {
            Term::Op(_, args) => {
                let Sort::Ff(order) = pool.sort(&args[0]).as_sort().cloned().unwrap() else {
                    unreachable!();
                };
                Some(Term::new_ffval(Integer::from(1), order))
            }
            _ => unreachable!(),
        },
        _ => None,
